...
def load_data():
    ...
    predictions=pd.read_csv(r'dataframes/predict_df_multi.csv',index_col=0)
    return prices,rGDP,eCPI,fCPI,predictions
prices,rGDP,eCPI,fCPI,predictions = load_data() #importing Dataframes
# # # # # # # # # # # # # # # DATA CLEANING # # # # # # # # # # # # # # # # # # # # #
...
# # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # 
# seaborn graph example
seaborn_chart=plt.figure(figsize=(24,8))
plot=sns.lineplot(prices_simple.wheat,label='Wheat Price  €')
plot.axvline(pd.to_datetime('1/31/2020'),c='r',label='WHO Issues Global Health Emergency',ymin=.1,ymax=.55)
plot.axvline(pd.to_datetime('2/24/2022'),c='black',label='Russia Invades Ukraine',ymin=.3,ymax=.98,)
plt.xlabel("")
plt.ylabel("Wheat Price  €")
props = dict(boxstyle='round', facecolor='#7393B3', alpha=0.5)
string1="Wheat prices begin\nan upward trend"
string2="Wheat prices\nhit a new record\nin March 2022"
text1=plot.text(0.7,0.4,string1,fontsize=14,transform=plot.transAxes,verticalalignment='center', bbox=props)
text2=plot.text(0.9,0.2,string2,fontsize=14,transform=plot.transAxes,verticalalignment='center', bbox=props)
plot.legend()